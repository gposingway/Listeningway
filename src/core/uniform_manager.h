...
public:
     ...
     * @param volume_left The current audio volume for the left channel, represented as a float (default 0.0f).
    ...
    void update_uniforms(reshade::api::effect_runtime* runtime, float volume, const std::vector<float>& freq_bands, float beat,
                        ...
                        float volume_left = 0.0f, float volume_right = 0.0f, float audio_pan = 0.0f);
};
